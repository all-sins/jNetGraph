...
@Component
public class SpeedtestCLIMeasurer {


    @Value("${speedtest.path}")
    private String speedtestPath;

    private final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(SpeedtestCLIMeasurer.class);
    ...
    public SpeedDataDTO getData() {
        ...
        Process process;
        try {
            process = Runtime.getRuntime().exec(speedtestPath + " -f json");
            process.waitFor();
